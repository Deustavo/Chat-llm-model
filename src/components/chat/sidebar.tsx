...
export default function Sidebar({ isResponsive, ...props }: SideBarProps) {
    ...
    return (
        <>
            ...
            <div
                className={`flex flex-col justify-between h-full p-2 text-white bg-gray-900 z-10 transition-all ease-in-out duration-500 ${isResponsive ? 'fixed left-0 top-0' : ''}`}
                        ...
                        {chat?.map(({ id, role }) => {
                            return (
                                <button
                                    id={id}
                                    key={id}
                                    className={`flex items-center p-2 rounded transition-all ease-in-out duration-500 ${selectedChat?.id == id ? "bg-white/20" : "bg-transparent"} hover:bg-white/10`}
